...
const Popup = () => {
    ...
    const [startTime, setStartTime] = useState({ hours: "10", minutes: "30", period: "AM" });
    const [endTime, setEndTime] = useState({ hours: "1", minutes: "30", period: "PM" });
    ...
    const togglePeriod = (timeType) => {
        if (timeType === "start") {
            ...
            setEndTime({ ...endTime, period: endTime.period === "AM" ? "PM" : "AM" });
        }
    };
    ...
    const customCaption = ({ displayMonth }: CaptionProps) => {
        ...
        const months = Array.from({ length: 12 }, (_, i) => format(setMonth(displayMonth, i), "MMMM"));
        const years =Array.from({ length: 6 }, (_, i) => startYear + i);

        return (
            <div className={styles.calendarCaption}>
                <div className={styles.calendarMonth} onClick={handleMonthClick}>
                    {format(displayMonth, "MMMM")}
                    {showMonthPicker && (
                        <div className={styles.monthPicker}>
                            {months.map((month, index) => (
                                <div key={month} onClick={() => handleMonthChange(index)}>
                                    {month}
                                </div>
                            ))}
                        </div>
    ...
    return (
        <>
            <button onClick={openPopup} className={styles.popupButton}>Create New Event</button>
            {visible && (
                        ...
                        <ul>
                            ...
                            <div className={styles.timePicker}>
                                <input
                                    ...
                                    onChange={(e) => handleStartTimeChange(e, "hours")}
                                ...
                                <input
                                    ...
                                    onChange={(e) => handleStartTimeChange(e, "minutes")}
                                    className={styles.timeInput}
                                    min={0}
                                    max={59}
                                />
                                <button onClick={() => togglePeriod("start")} className={styles.periodButton}>
                                    {startTime.period}
                                </button>
                                ...
                                <input
                                    type="number"
                                    value={endTime.hours}
                                    onChange={(e) => handleEndTimeChange(e, "hours")}
                                    className={styles.timeInput}
                                    min={1}
                                    max={12}
                                />
                                ...
                                <input
                                    ...
                                    onChange={(e) => handleEndTimeChange(e, "minutes")}
                                    className={styles.timeInput}
                                    min={0}
                                    max={59}
                                />
                            ...
                            <div className={styles.submitCancelContainer}>
                                ...
                                <button onClick={closePopup} className={styles.submitCancelButton}>cancel</button>
                            </div>
                        </ul>
